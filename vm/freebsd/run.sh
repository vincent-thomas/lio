...
case "$VM_ARCH" in
    x86_64)
        ...
        IMAGE_URL="https://download.freebsd.org/releases/VM-IMAGES/${FREEBSD_VERSION}-RELEASE/amd64/Latest/FreeBSD-${FREEBSD_VERSION}-RELEASE-amd64-BASIC-CLOUDINIT-ufs.qcow2.xz"
        ;;
    aarch64)
        QEMU_BIN="qemu-system-aarch64"
        IMAGE_URL="https://download.freebsd.org/releases/VM-IMAGES/${FREEBSD_VERSION}-RELEASE/aarch64/Latest/FreeBSD-${FREEBSD_VERSION}-RELEASE-arm64-aarch64-BASIC-CLOUDINIT-ufs.qcow2.xz"
        ;;
esac

IMAGE_FILE="$CACHE_DIR/freebsd-${FREEBSD_VERSION}-${VM_ARCH}.qcow2"
...
Test lio on FreeBSD VM with kqueue backend.
...
download_image() {
    ...
    if [[ ! -f "$IMAGE_FILE" ]]; then
        echo "Downloading FreeBSD ${FREEBSD_VERSION} cloud image for ${VM_ARCH}..."
        curl -L -o "$IMAGE_FILE.xz" "$IMAGE_URL"
...
EOF
    ...
    # FreeBSD uses nuageinit which has limited cloud-init support
    # The 'freebsd' user exists by default in cloud images
    # Use pkg for Rust since rustup doesn't support aarch64-freebsd
...
create_source_disk() {
    ...
    local src_disk="$CACHE_DIR/lio-src.img"
...
EOF
    ...
    # FreeBSD: extract source from secondary disk (raw tar)
    # Rust is installed via pkg, not rustup, so no .cargo/env needed
    cat > "$user_data" <<'CLOUDINIT'
#cloud-config

runcmd:
  - echo "=== Extracting source from disk ==="
  - rm -rf /home/freebsd/lio
...
runcmd:
  - mkdir -p /mnt/lio
  - mount -t virtfs -o trans=virtio lio-src /mnt/lio || mount_9p lio-src /mnt/lio || true
CLOUDINIT
...
run_vm() {
    ...
    echo "Starting FreeBSD VM ($VM_ARCH)..."
    ...
    case "$VM_ARCH" in
        aarch64)
            if [[ -z "${QEMU_EFI_AARCH64:-}" ]]; then
                echo "Error: QEMU_EFI_AARCH64 not set"
                echo "Run via: nix run .#freebsd"
                exit 1
            fi
            qemu_cmd+=(
                -M virt
...
if $CLEAN_MODE; then
    ...
    rm -f "$IMAGE_FILE" "$PROVISIONED_FILE" "$DISK_FILE" "$SEED_FILE" "$CACHE_DIR/lio-src.img"
...
VM_OUTPUT_FILE="$CACHE_DIR/vm-output-freebsd.log"

if $SHELL_MODE; then
    create_shell_seed
    create_source_disk
    echo "Launching VM in interactive mode..."
    echo "Login: freebsd / freebsd"
    echo "Source at /home/freebsd/lio (extract: dd if=/dev/vtbd1 | tar -xf - -C ~/lio)"

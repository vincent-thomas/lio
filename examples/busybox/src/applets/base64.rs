...
use crate::{
  app::AppContext, applets::support::stream_base64_output, command::Command,
  util::io as io_util,
};
...
impl Command for Base64Command {
  ...
  fn execute(&self, ctx: &AppContext) -> io::Result<()> {
    ...
    stream_base64_output(ctx.lio(), &input, &ctx.stdout(), 76)
  }
}
...
mod tests {
  use super::*;
  use crate::applets::support::{encode_base64, stream_base64_output};
  use std::{ffi::CString, os::fd::FromRawFd, path::PathBuf};

  #[test]
  fn parse_base64_accepts_optional_file() {
    let parsed = Base64Command::parse(&["file".into()]).unwrap();
    assert_eq!(parsed.path.as_deref(), Some("file"));
  }

  #[test]
  fn streaming_base64_matches_previous_output() {
    let ctx = AppContext::new().unwrap();
    let data = b"hello world, hello world, hello world, hello world";
    let input_path = temp_path("base64");

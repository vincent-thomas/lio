...
impl ReceiverNode {
  ...
  fn drive(&mut self, bind_addr: SocketAddr) {
    ...
    {
      self.bind_rx = None;
      let socket = self.socket.as_ref().expect("receiver socket bound");
...
impl SenderNode {
  ...
  fn drive(&mut self, bind_addr: SocketAddr, peer_addr: SocketAddr) {
    ...
    {
      self.bind_rx = None;
      let socket = self.socket.as_ref().expect("sender socket bound");
...
#[test]
fn socket_sendto_and_recvfrom_round_trip() {
  let recv_addr: SocketAddr = "127.0.0.1:7000".parse().unwrap();
  let send_addr: SocketAddr = "127.0.0.1:7001".parse().unwrap();

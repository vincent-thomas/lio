...
doc_op! {
    ...
    {
        Io::from_op(ops::Read::new(res.as_resource().clone(), buf))
    }
}
...
doc_op! {
    ...
    where
        B: IoBufMutVec + Send + Sync + 'static,
    {
        Io::from_op(ops::Read::new(res.as_resource().clone(), buf).at(offset))
    }
}
...
doc_op! {
    ...
    where
        B: IoBufVec + Send + Sync + 'static,
    {
        Io::from_op(ops::Write::new(res.as_resource().clone(), buf).at(offset))
    }
}
...
//   mask: ops::WatchMask,
// ) -> io::IoStream<ops::WatchStream> {
//   let path_cstring = CString::new(path.as_ref().as_os_str().as_encoded_bytes())
//     .expect("path contains null byte");
//   io::IoStream::from_op(ops::WatchStream::new(path_cstring, mask))
// }
//
// doc_op!(
...
// ///
// /// Returns an [`IoStream`](io::IoStream) that yields connections
...
//   res: &impl AsResource,
// ) -> io::IoStream<ops::AcceptStream> {
//   io::IoStream::from_op(ops::AcceptStream::new(

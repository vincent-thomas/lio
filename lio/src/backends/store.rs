//! Thread-safe storage for in-flight I/O operations.
//!
//! This module provides the [`OpStore`], a lock-free concurrent data structure for
//! managing in-flight I/O operations. It uses generational indices to prevent ABA
//! problems and supports efficient concurrent access.
//!
//! # Design
//!
//! The store uses a generational index scheme where each ID is composed of:
//! - **Slot**: The location in the underlying storage (low 32 bits)
...
//!
//! ```rust,ignore
//! use lio::backends::OpStore;
//! use lio::registration::StoredOp;
//!
//! let store = OpStore::with_capacity(1024);
//!
//! // Insert an operation
//! let id = store.insert(stored_op);
//!
//! // Access it
//! store.get_mut(id, |registration| {
...
/// Internal record storing a registration and its generation.
struct Record {
...
///
/// This allows detecting stale references (ABA problem protection).
...
impl Index {
  ...
  /// Creates a new index with incremented generation.
  ///
  /// # Panics
  ///
  /// Panics if the generation counter overflows.
...
/// Thread-safe store for in-flight I/O operations.
///
/// `OpStore` provides lock-free concurrent access to operation registrations using
/// generational indices. This prevents the ABA problem where an ID might be reused
/// and accidentally refer to a different operation.
///
/// # Thread Safety
///
/// All methods are safe to call concurrently from multiple threads. The store uses
/// lock-free algorithms internally for maximum performance.
///
/// # Capacity
///
/// The store has a fixed capacity that must be a power of 2. When creating a store,
/// use [`with_capacity`](Self::with_capacity) and ensure the capacity is sufficient
/// for your workload.
pub struct OpStore {
...
impl OpStore {
  ...
  ///
  /// This is only available in tests. Production code should use
  ...
  /// # Parameters
  ///
  /// - `cap`: The maximum number of concurrent operations. Must be a power of 2.
  ///
  /// # Panics
  ///
  /// Panics if `cap` is not a power of 2.
  ///
  /// # Examples
  ///
  /// ```rust,ignore
  /// use lio::backends::OpStore;
  ///
  /// let store = OpStore::with_capacity(2048);
  /// ```
  ...
  ///
  /// This either reuses a freed slot (with incremented generation) or
  ...
  /// # Parameters
  ///
  /// - `stored`: The operation to insert
  ///
  /// # Returns
  ///
  /// A unique u64 ID that can be used to access or remove the operation.
  ///
  /// # Examples
  ///
  /// ```rust,ignore
  /// let id = store.insert(stored_op);
  /// // Use the ID later
  /// store.get_mut(id, |reg| { /* ... */ });
  /// ```
  pub fn insert(&self, stored: StoredOp) -> u64 {
    self.insert_with(|_| stored)
  }

  /// Inserts an operation, passing the allocated ID to the closure.
  ///
  /// This is useful when the operation needs to know its own ID during creation.
  ///
  /// # Parameters
  ///
  /// - `f`: Closure that receives the allocated ID and returns the operation
  ///
  /// # Returns
  ///
  /// The unique u64 ID for the inserted operation.
  ...
  ///
  /// Similar to [`insert_with`](Self::insert_with), but allows the initialization
  /// closure to fail.
  ///
  /// # Parameters
  ///
  /// - `f`: Closure that receives the allocated ID and returns `Result<StoredOp, E>`
  ///
  /// # Returns
  ///
  /// - `Ok(u64)`: The unique ID on success
  /// - `Err(E)`: The error from the closure on failure
  ...
  ///
  /// This frees the slot for reuse. The generation counter is incremented
  /// when the slot is reused, preventing stale IDs from accessing the new
  /// operation.
  ///
  /// # Parameters
  ///
  /// - `id`: The operation ID to remove
  ///
  /// # Returns
  ///
  /// - `true`: The operation was found and removed
  /// - `false`: The ID was not found (either never existed or already removed,
  ///   or refers to a stale generation)
  ///
  /// # Examples
  ///
  /// ```rust,ignore
  /// let id = store.insert(op);
  /// assert!(store.remove(id));  // Returns true
  /// assert!(!store.remove(id)); // Returns false (already removed)
  /// ```
  ...
  /// Accesses an operation with mutable access to its registration.
  ///
  /// The closure is called with a mutable reference to the operation's
  /// registration if the ID is valid and current.
  ///
  /// # Parameters
  ///
  /// - `id`: The operation ID
  /// - `f`: Closure to call with the registration
  ///
  /// # Returns
  ///
  /// - `Some(R)`: The closure's return value if the ID was valid
  /// - `None`: The ID was not found or refers to a stale generation
  ///
  /// # Examples
  ///
  /// ```rust,ignore
  /// store.get_mut(id, |registration| {
  ///     registration.set_done(result);
  /// });
  /// ```
  ...
  /// Accesses an operation with read-only access to its registration.
  ///
  /// The closure is called with a reference to the operation's registration
  /// if the ID is valid and current.
  ///
  /// # Parameters
  ///
  /// - `id`: The operation ID
